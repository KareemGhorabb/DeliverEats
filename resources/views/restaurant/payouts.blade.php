...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    <div class="bg-white rounded-2xl border border-surface-200/50 p-5">
        ...
        <p class="text-2xl font-display font-bold text-emerald-600">EGP 12,847</p>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-surface-100">
                ...
                <tr class="hover:bg-surface-50"><td class="px-6 py-4 font-medium">{{ $p['date'] }}</td><td class="px-6 py-4 text-surface-300">{{ $p['period'] }}</td><td class="px-6 py-4">{{ $p['orders'] }}</td><td class="px-6 py-4">EGP {{ $p['gross'] }}</td><td class="px-6 py-4 text-red-500">-${{ $p['commission'] }}</td><td class="px-6 py-4 font-bold">EGP {{ $p['net'] }}</td><td class="px-6 py-4"><span class="px-2.5 py-1 text-[11px] font-bold rounded-full {{ $p['color'] }}">{{ $p['status'] }}</span></td></tr>
                @endforeach
            </tbody>
        </table>
    </div>
...
<div class="mt-6 bg-blue-50 border border-blue-200 rounded-2xl p-5 flex items-start gap-3">
    ...
    <div><p class="text-sm font-semibold text-blue-800">Paymob</p><p class="text-xs text-blue-700/70 mt-0.5">Payouts are processed every Tuesday via Paymob. Funds are deposited directly to your bank account within 2-3 business days.</p></div>
</div>
@endsection
